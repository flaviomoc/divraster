...
# data.frame
traits <- read.csv(system.file("extdata", "traits_Mota.csv", package = "DMSD"), sep = ";")
```
...
```{r, fig.height = 4, fig.width = 5, fig.align = 'center',}
# ecological generalization for the trait wing length
wing.ref <- ecogen(ref, traits$Wing.Length) # reference scenario
wing.ref
terra::plot(wing.ref, main = names(wing.ref))

wing.fut <- ecogen(fut, traits$Wing.Length) # future scenario
wing.fut
terra::plot(wing.fut, main = names(wing.fut))
```
