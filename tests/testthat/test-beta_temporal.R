test_that("function temp.beta works", {
  ...
  # testing
  expect_equal(round(beta.td[[1]]@pnt$range_max, 2), .88)
  expect_equal(round(beta.td[[2]]@pnt$range_max, 2), .86)
  expect_equal(round(beta.td[[3]]@pnt$range_max, 2), .62)
  expect_equal(round(beta.fd[[1]]@pnt$range_max, 2), .75)
  expect_equal(round(beta.fd[[2]]@pnt$range_max, 2), .72)
  expect_equal(round(beta.fd[[3]]@pnt$range_max, 2), .52)
  expect_equal(round(beta.pd[[1]]@pnt$range_max, 2), .72)
  expect_equal(round(beta.pd[[2]]@pnt$range_max, 2), .58)
  expect_equal(round(beta.pd[[3]]@pnt$range_max, 2), .49)
})
