test_that("function spat.beta works", {
  ...
  # testing
  expect_equal(round(beta.td[[1]]@pnt$range_max, 2), .83)
  expect_equal(round(beta.td[[2]]@pnt$range_max, 2), .72)
  expect_equal(round(beta.td[[3]]@pnt$range_max, 2), .37)
  expect_equal(round(beta.fd[[1]]@pnt$range_max, 2), .74)
  expect_equal(round(beta.fd[[2]]@pnt$range_max, 2), .65)
  expect_equal(round(beta.fd[[3]]@pnt$range_max, 2), .26)
  expect_equal(round(beta.pd[[1]]@pnt$range_max, 2), .68)
  expect_equal(round(beta.pd[[2]]@pnt$range_max, 2), .54)
  expect_equal(round(beta.pd[[3]]@pnt$range_max, 2), .31)
})
